...
Watcher.prototype = {
  ...
  addDep: function(dep) {
    ...
    // 间接调用了它的getter 触发了addDep(), 在整个forEach过程，
    // 当前wacher都会加入到每个父级过程属性的dep
    // 例如：当前watcher的是'child.child.name', 
    // 那么child, child.child, child.child.name这三个属性的dep都会加入当前watcher
...
// 实例化Watcher的时候,调用get()方法,通过Dep.target=watcherInstance标记订阅者是当前Watcher的实例
